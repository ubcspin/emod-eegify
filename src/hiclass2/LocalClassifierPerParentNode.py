...
from hiclass2.ConstantClassifier import ConstantClassifier
from hiclass2.HierarchicalClassifier import HierarchicalClassifier
...
class LocalClassifierPerParentNode(BaseEstimator, HierarchicalClassifier):
    ...
    def _initialize_local_classifiers(self):
        ...
        for node in nodes:
            if node == self.root_:
                root_clsf = deepcopy(self.local_classifier_)
                root_clsf.set_params(module__n_labels=self.root_classes)
                local_classifiers[node] = {"classifier": root_clsf}
                continue
    ...
    def _fit_classifier(self, node):
        ...
        fn = lambda x: (int(x[0]), int(x[-1]))
        class_fn = lambda x: x[-1] if node != self.root_ else x[0]
        yn = np.array([ class_fn(fn(x)) for x in y])
